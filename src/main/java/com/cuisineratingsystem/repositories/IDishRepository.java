...
public interface IDishRepository extends JpaRepository<Dish, Integer> {
	...
	@Query("select c from Dish c where c.dish_name like %?1%")
	 ...
	 @Query("select d.dish_ID,d.rating,d.no_of_raters,d.dish_name, d.cuisine.restaurant.rest_name, d.imageURL from Dish d where rating > 3 order by rating DESC, no_of_raters DESC")
